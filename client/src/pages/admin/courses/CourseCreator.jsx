...
export default function CourseCreator() {
    ...
    const handleGenerateBlueprint = async () => {
        if (!engineConfig?.architect?.model)
            return showToast(
                "Go to AI Command Center and deploy routing config first.",
                "error",
            );
        if (!youtubeUrl && files.length === 0)
            return showToast(
                "Please provide a YouTube URL or upload context files.",
                "warning",
            );
    ...
    const handleRefineBlueprint = async () => {
        ...
        setLoading(true);
        try {
            const res = await api.refineBlueprint(
                blueprint,
                blueprintPrompt,
                engineConfig.architect,
            );
    ...
    const handleRefineSteps = async () => {
        ...
        try {
            ...
            const res = await api.refineModuleSteps(
                currentSteps,
                stepPrompt,
                engineConfig.lesson,
            );
            ...
            {step === 1 && (
                <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-200">
                    <h2 className="text-2xl font-black text-gray-900 mb-6 flex items-center gap-2">
                        <Target className="text-indigo-600" /> Source Ingestion
                    </h2>
                    <div className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input
                                    type="text"
                                    placeholder="e.g. Master PowerBI DAX"
                                    className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl outline-none font-bold focus:border-indigo-500"
                        ...
                        <div className="pt-4 border-t border-gray-100">
                            ...
                            <input
                                ...
                                className="w-full p-4 bg-red-50/50 border border-red-100 rounded-xl outline-none focus:border-red-300"
                                value={youtubeUrl}
                                onChange={(e) => setYoutubeUrl(e.target.value)}
                            />
                            <p className="text-[10px] text-gray-400 mt-2 font-medium">
                                Vertex AI will watch this video and map the
                                timestamps natively.
                            </p>
                        </div>
                        ...
                        <div>
                            ...
                            >
                                ...
                                <p className="font-bold text-indigo-900 text-sm">
                                    Drag & drop PDFs, TXT, or Code files
                                </p>
                            ...
                            {files.length > 0 && (
                                <div className="mt-4 flex flex-wrap gap-2">
                                    {files.map((file, i) => (
                                        <div
                                            key={i}
                                            className="flex items-center gap-2 bg-white border border-gray-200 px-3 py-1.5 rounded-lg text-xs font-bold shadow-sm"
                                        >
                        ...
                        <button
                            ...
                            disabled={loading}
                            className="w-full py-4 mt-4 bg-gray-900 text-white rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-black disabled:opacity-50 shadow-lg"
                        >
                            {loading ? (
                    ...
                    <div className="bg-white p-8 rounded-3xl shadow-sm border border-indigo-200">
                        <div className="flex justify-between items-start mb-6">
                            <div>
                                ...
                                <span className="text-xs font-bold text-indigo-600 uppercase tracking-widest mt-1 block">
                                    AI Draft Blueprint
                                </span>
                            </div>
                        </div>
                        ...
                        <div className="space-y-3 mb-8">
                            {blueprint.modules.map((m, i) => (
                                <div
                                    key={i}
                                    className={`p-5 rounded-2xl border flex flex-col sm:flex-row gap-4 items-start sm:items-center ${m.is_static ? "bg-amber-50/30 border-amber-100" : "bg-gray-50 border-gray-100"}`}
                                >
                                    <div className="bg-white text-indigo-600 w-10 h-10 rounded-xl flex items-center justify-center font-black shadow-sm shrink-0 border border-gray-100">
                                        {i + 1}
                                    </div>
                                    ...
                                    <div className="flex-1">
                                        <div className="flex items-center gap-2">
                                            ...
                                            {m.is_static && (
                                                <span className="bg-amber-100 text-amber-700 text-[10px] px-2 py-0.5 rounded font-bold uppercase tracking-widest">
                                                    Career Launchpad
                                                </span>
                                            )}
                                        </div>
                        ...
                        <div className="bg-indigo-50/50 border border-indigo-100 p-4 rounded-2xl flex gap-3 mb-8">
                            ...
                            />
                            <div className="flex-1">
                                <label className="text-[10px] font-black text-indigo-800 uppercase tracking-widest block mb-1">
                                    AI Revision Command
                                </label>
                                <div className="flex gap-2">
                                    <input
                                        ...
                                        onChange={(e) =>
                                            setBlueprintPrompt(e.target.value)
                                        }
                                        onKeyDown={(e) =>
                                            e.key === "Enter" &&
                                    ...
                                    <button
                                        ...
                                        disabled={loading || !blueprintPrompt}
                                        className="bg-indigo-600 text-white px-4 py-2.5 rounded-lg text-sm font-bold hover:bg-indigo-700 disabled:opacity-50 flex items-center gap-2"
                                    >
                                        {loading ? (
                                            <Loader2
                                                size={16}
                        ...
                        <button
                            ...
                            className="w-full py-4 bg-gray-900 text-white rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-black disabled:opacity-50 shadow-xl"
                        >
                            {generatingAll ? (
                                <Loader2 className="animate-spin" />
                            ) : (
                            ...
                            Approve & Auto-Extract UI Action Maps
                        </button>
                    </div>
                </div>
            )}
                        ...
                        <div className="flex-1 overflow-y-auto p-4 space-y-2">
                            {blueprint.modules.map((mod, idx) => (
                                <button
                                    ...
                                    className={`w-full text-left p-4 rounded-xl border transition-all ${activeModuleIdx === idx ? "bg-indigo-50 border-indigo-200 shadow-sm" : "bg-white border-gray-100 hover:border-gray-300"}`}
                                >
                                    ...
                                    <h4
                                        className={`font-bold text-sm ${activeModuleIdx === idx ? "text-indigo-900" : "text-gray-700"} line-clamp-2`}
                                    >
                                        {mod.title}
                                    </h4>
                                </button>
                    ...
                    <div className="flex-1 bg-white border border-gray-200 rounded-3xl shadow-sm flex flex-col overflow-hidden">
                        <div className="p-5 border-b flex justify-between items-center bg-gray-50/50">
                            ...
                            <button
                                ...
                                disabled={loading}
                                className="bg-emerald-600 text-white px-5 py-2 rounded-xl font-bold flex items-center gap-2 hover:bg-emerald-700 shadow-md"
                            >
                                {loading ? (
                                    <Loader2
                                        size={16}
                        ...
                        <div className="flex-1 overflow-y-auto p-6 bg-[#f8fafc]">
                            {!moduleContents[activeModuleIdx] ? (
                                ...
                                <div className="space-y-4 max-w-3xl mx-auto">
                                    {moduleContents[activeModuleIdx].map(
                                        (stepObj, sIdx) => (
                                            <div
                                                key={sIdx}
                                            ...
                                            >
                                                <div
                                                    className={`px-4 py-2 border-b flex items-center gap-2 ${stepObj.step_type === "action_trigger" ? "bg-purple-50/50 border-purple-100" : "bg-blue-50/50 border-blue-100"}`}
                                                >
                                                    <span className="bg-white text-gray-800 w-6 h-6 rounded-md flex items-center justify-center text-xs font-black shadow-sm border border-gray-100">
                                                        {sIdx + 1}
                                                    </span>
                                                    {stepObj.step_type ===
                                                    "action_trigger" ? (
                                                ...
                                                <div className="p-4 space-y-4">
                                                    <div>
                                                        ...
                                                        <textarea
                                                            rows="2"
                                                            className="w-full bg-gray-50 border border-gray-200 rounded-lg p-3 text-sm outline-none focus:border-indigo-400 resize-none font-medium"
                                                            value={
                                                                stepObj.instruction_text
                                                            }
                                                            onChange={(e) =>
                                                                handleUpdateStep(
                                                                    sIdx,
                                                    ...
                                                    {stepObj.step_type ===
                                                        ...
                                                        <div className="bg-gray-50 p-4 rounded-xl border border-gray-200 space-y-3">
                                                            <div>
                                                                ...
                                                                <input
                                                                    type="text"
                                                                    className="w-full bg-white border border-gray-200 rounded-lg p-2.5 text-sm font-mono text-indigo-700 outline-none"
                                                                    value={
                                                                        stepObj
                                                                            .ui_target
                                                                            ?.description ||
