        ...
        export const api = {
            ...
            getYoutubeMetadata: async (url) => {
                const res = await fetchAPI('/api/ai/youtube-meta', {
                    method: 'POST',
                    body: JSON.stringify({ url })
                });
                return res.metadata; // { title, duration }
            },

            // 2. Blueprint Generation
            generateBlueprint: async (topic, audience, engineConfig, knowledgeSources) => {
                const res = await fetchAPI('/api/ai/generate-blueprint', {
                    method: 'POST',
                    body: JSON.stringify({ topic, audience, engineConfig, knowledgeSources })
                });
                return res; // { blueprint, usage }
            },

            // 3. Module Step Extraction (Batch Processing)
            ...
            syncCourseToBrain: async (courseId, embedderConfig) => {
                return await fetchAPI('/api/ai/brain/sync', {
                    method: 'POST',
                    body: JSON.stringify({ courseId, embedderConfig })
                });
            },
