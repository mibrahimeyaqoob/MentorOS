...
export const addKey = async (req, res, next) => {
    try {
        ...
        // Mask: "AIzaSy...5G9s"
        const masked_key = `${key.substring(0, 6)}...${key.substring(key.length - 4)}`;
...
export const getRouting = async (req, res, next) => {
    try {
        let { data, error } = await supabase.from('routing_config').select('*').single();
...
export const getAuditLogs = async (req, res, next) => {
    try {
        ...
        const logs = [
            { id: 1, created_at: new Date(), users: { name: 'Admin' }, action: 'SYSTEM_INIT', entity_name: 'MentorOS Core', tokens_used: 0 }
        ];
